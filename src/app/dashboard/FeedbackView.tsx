      ...
      <div style={{ paddingTop: isMobile ? 56 : 40, paddingRight: isMobile ? 16 : 40, paddingBottom: isMobile ? 16 : 40, paddingLeft: isMobile ? 16 : 40, maxWidth: 520 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #EFEFEF", borderRadius: 16, padding: 28, display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", gap: 4 }}>
            {[1,2,3,4,5].map(s => (
              <span key={s} style={{ fontSize: 28, color: s <= existing.rating ? "#F59E0B" : "#E5E5E5" }}>★</span>
            ))}
          </div>
          {existing.message && (
            <p style={{ fontSize: 14, color: "#1A1A1A", margin: 0, lineHeight: 1.6 }}>{existing.message}</p>
          )}
          <button
            type="button"
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[1,2,3,4,5].map((star) => (
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: 32, color: star <= (hovered || rating) ? "#F59E0B" : "#E5E5E5", transition: "color 0.1s" }}
              >★</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {editing && (
            <button
              ...
              style={{ background: "none", border: "1px solid #E5E5E5", borderRadius: 10, padding: "12px 20px", fontSize: 14, fontWeight: 600, fontFamily: "inherit", cursor: "pointer", color: "#1A1A1A" }}
            >Cancel</button>
          )}
          <button
            type="button"
