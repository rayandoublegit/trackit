...
import { PIPELINE_STAGES, stageColor } from "@/lib/pipeline";
import {
...
function MiniCard({ r, onOpen, draggable }: { r: SavedRow; onOpen: () => void; draggable?: boolean }) {
  ...
  return (
    <div
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: r.value_score >= 70 ? "#15803D" : r.value_score >= 40 ? "#B45309" : "#9A1F1F" }}>Renta {r.value_score}</span>
        <span style={{ fontSize: 10, color: "#7A7A7A" }}>· {fmt(r.followers)} ab.</span>
      ...
      <span style={{ alignSelf: "flex-start", fontSize: 10, fontWeight: 600, color: sc.color, background: sc.bg, padding: "2px 8px", borderRadius: 20 }}>
        {PIPELINE_STAGES.find((s) => s.key === r.pipeline_status)?.label ?? r.pipeline_status}
      </span>
    </div>
  );
}

export function MyCreatorsView({ plan, isMobile, onUpgrade }: { plan: PlanTier; isMobile?: boolean; onUpgrade: () => void }) {
  ...
  if (!isPaid) {
    return (
      <div style={{ padding: pad, background: "#FFF", minHeight: "100vh" }}>
        <h1 style={{ fontSize: 28, fontWeight: 600, color: "#1A1A1A", letterSpacing: "-0.04em", margin: 0 }}>Mes créateurs</h1>
        <p style={{ fontSize: 14, color: "#7A7A7A", margin: "6px 0 24px" }}>Sauvegarde des créateurs, range-les en dossiers et suis ton pipeline d&apos;outreach.</p>
        <div style={{ border: "1px solid #EFEFEF", borderRadius: 16, padding: "32px 28px", textAlign: "center", maxWidth: 420 }}>
          <div style={{ fontSize: 18, fontWeight: 600, color: "#1A1A1A", marginBottom: 8 }}>Réservé aux plans payants</div>
          <div style={{ fontSize: 14, color: "#7A7A7A", marginBottom: 18, lineHeight: 1.5 }}>Les dossiers et le pipeline CRM (contacté, en cours, signé…) sont inclus dans les plans payants.</div>
          <button type="button" onClick={onUpgrade} style={{ background: "#0047FF", color: "#FFF", border: "none", borderRadius: 12, padding: "12px 22px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Passer à un plan payant</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: pad, background: "#FFF", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", margin: "18px 0 22px" }}>
        ...
        {folders.map((f) => {
          ...
          return (
            <span key={f.id} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: active ? "#E8EEFC" : "#F5F5F5", borderRadius: 20, padding: "0 4px 0 0" }}>
              ...
              <button type="button" aria-label={`Supprimer ${f.name}`} onClick={() => onDeleteFolder(f.id)}
        ...
        <span style={{ display: "inline-flex", gap: 4 }}>
          <input value={newFolder} onChange={(e) => setNewFolder(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") onCreateFolder(); }}
            placeholder="+ dossier" style={{ fontSize: 12, padding: "5px 10px", borderRadius: 20, border: "1px solid #E5E5E5", width: 110, fontFamily: "inherit" }} />
        </span>
      </div>

      {loading ? (
        <div style={{ color: "#9A9A9A", fontSize: 14 }}>Chargement…</div>
      ) : rows.length === 0 ? (
        <div style={{ color: "#9A9A9A", fontSize: 14 }}>Aucun créateur sauvegardé. Va dans Recherche et clique « Sauver » sur un créateur.</div>
      ) : mode === "list" ? (
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(260px, 1fr))", gap: 14 }}>
          {visibleRows.map((r) => <MiniCard key={r.creator_username} r={r} onOpen={() => setSelected(rowToCreator(r))} />)}
        </div>
      ) : (
            ...
            return (
              <div key={s.key} onDragOver={(e) => e.preventDefault()} onDrop={onDrop(s.key)}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {colRows.map((r) => <MiniCard key={r.creator_username} r={r} draggable onOpen={() => setSelected(rowToCreator(r))} />)}
                </div>
              </div>
            );
          })}
      ...
      <CreatorDetailDrawer creator={selected} plan={plan} onClose={() => setSelected(null)} onUpgrade={onUpgrade} onWorkspaceChange={load} />
    </div>
  );
}
