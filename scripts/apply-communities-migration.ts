/**
 * Applique la migration Communautés sur Supabase Trackit.
 * SQL: supabase/migrations/20260825_000036_communities.sql
 */
...
const sqlFile = path.resolve(__dirname, "../supabase/migrations/20260825_000036_communities.sql");
...
async function verify(): Promise<boolean> {
  ...
  const { error: c } = await sb.from("communities").select("id").limit(1);
  if (c) return false;
  const { error: m } = await sb.from("community_members").select("community_id").limit(1);
  if (m) return false;
  const { error: msg } = await sb.from("community_messages").select("id").limit(1);
  return !msg;
}
...
async function main() {
  ...
  if (await verify()) {
    console.log("✓ Migration Communautés déjà appliquée.");
    return;
  }
  if (!dbUrl) {
    console.error("✗ Schema Communautés absent.\n");
    if (projectRef) {
  ...
  if (await verify()) console.log("✓ Migration Communautés appliquée.");
  else {
